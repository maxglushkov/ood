...
public:
	...
	{
		const auto savedFlags = std::cout.setf(std::ios::hex, std::ios::basefield);
		std::cout << "SetColor (#" << std::setfill('0') << std::setw(6) << (rgbColor & 0xffffff) << ')' << std::endl;
		std::cout.setf(savedFlags);
	}
